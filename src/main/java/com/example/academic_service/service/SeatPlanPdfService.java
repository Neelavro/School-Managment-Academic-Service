...
public class SeatPlanPdfService {
    ...
    private static final int CARD_W         = 357;
    private static final int CARD_H         = 265;
    ...
    public List<EnrollmentResponseDto> fetchEnrollments(
    ...
    // Map<enrollmentId, roomName> for enrollments whose classRoll falls inside a
    // room's [startRoll, endRoll]. Also returns a sort key (startRoll of that
    // room) so students of the same room stay together.
    public RoomAssignmentIndex buildRoomIndex(Integer routineId, Integer classId) {
        if (routineId == null || classId == null) return RoomAssignmentIndex.empty();
        List<ExamClassRoomAssignment> assignments = roomAssignmentRepository
                .findByExamRoutineIdAndExamClassId(routineId, classId);
        return new RoomAssignmentIndex(assignments);
    }
    ...
    public byte[] generatePdf(List<EnrollmentResponseDto> enrollments,
                              String examName,
                              RoomAssignmentIndex roomIndex) {
        try (Playwright playwright = Playwright.create()) {
            Browser browser = playwright.chromium().launch(
                    new BrowserType.LaunchOptions().setHeadless(true));
            Page page = browser.newPage();
            page.setContent(buildHtml(enrollments, examName, roomIndex),
    ...
    private double estimateNameWidth(String name) {
        double total = 0;
        for (char c : name.toCharArray()) total += getCharWeight(c);
        return total;
    }
    ...
    private String buildHtml(List<EnrollmentResponseDto> enrollments,
        ...
        for (int i = 0; i < enrollments.size(); i++) {
            ...
            String roomName  = roomIndex.roomNameFor(s.getClassRoll());
            ...
            int rollRowSpan = 2 + (groupName != null ? 1 : 0) + (roomName != null ? 1 : 0);
            ...
            if (roomName != null) {
                currentPage.append("<tr><td class=\"lbl\">Room</td><td class=\"sep\">:</td><td class=\"val\"><b>").append(roomName).append("</b></td></tr>");
            }
    ...
    // Sortable key + name lookup for a given classRoll based on the room
    // assignments of one (routine, class).
    public static class RoomAssignmentIndex {
        private final List<ExamClassRoomAssignment> sortedAssignments;

        RoomAssignmentIndex(List<ExamClassRoomAssignment> assignments) {
            this.sortedAssignments = assignments.stream()
                    .sorted(Comparator.comparingInt(a ->
                            a.getStartRoll() != null ? a.getStartRoll() : Integer.MAX_VALUE))
                    .collect(Collectors.toList());
        }

        static RoomAssignmentIndex empty() {
            return new RoomAssignmentIndex(Collections.emptyList());
        }

        public boolean isEmpty() { return sortedAssignments.isEmpty(); }

        private ExamClassRoomAssignment findFor(Integer classRoll) {
            if (classRoll == null) return null;
            for (ExamClassRoomAssignment a : sortedAssignments) {
                Integer start = a.getStartRoll();
                Integer end = a.getEndRoll();
                boolean inRange = (start == null || classRoll >= start)
                        && (end == null || classRoll <= end);
                if (inRange) return a;
            }
            return null;
        }

        public String roomNameFor(Integer classRoll) {
            ExamClassRoomAssignment a = findFor(classRoll);
            if (a == null || a.getRoom() == null) return null;
            return a.getRoom().getName();
        }

        // Sort key: startRoll of the room a student belongs to; students without
        // a room fall to the end.
        public int sortKeyFor(Integer classRoll) {
            ExamClassRoomAssignment a = findFor(classRoll);
            if (a == null || a.getStartRoll() == null) return Integer.MAX_VALUE;
            return a.getStartRoll();
        }
    }
}
