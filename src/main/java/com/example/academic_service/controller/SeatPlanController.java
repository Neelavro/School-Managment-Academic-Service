...
public class SeatPlanController {
    ...
    public ResponseEntity<byte[]> generateSeatPlan(
        ...
        SeatPlanPdfService.RoomAssignmentIndex roomIndex =
                seatPlanPdfService.buildRoomIndex(routineId, classId);

        enrollments.sort(Comparator
                .comparingInt((EnrollmentResponseDto e) ->
                        roomIndex.sortKeyFor(e.getClassRoll()))
                .thenComparingInt(e ->
                        e.getClassRoll() != null ? e.getClassRoll() : 0));

        byte[] pdf = seatPlanPdfService.generatePdf(enrollments, examName, roomIndex);
